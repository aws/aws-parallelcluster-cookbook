...
describe 'manage_dna_files resource' do
  for_all_oses do |platform, version|
    context "on #{platform}#{version}" do
      ...
      context "when we share dna files" do
        cached(:chef_run) do
          runner = runner(platform: platform, version: version, step_into: ['manage_dna_files']) do |node|
        ...
        it 'runs manage_fleet_dna.py to share dna files with both fleets' do
          is_expected.to run_execute('Run manage_fleet_dna.py to get user_data.sh and share dna.json with ComputeFleet and LoginNodes').with(
            command: "#{cookbook_virtualenv_path}/bin/python #{node['cluster']['scripts_dir']}/manage_fleet_dna.py" \
              " --region #{node['cluster']['region']}"
          )
        end
      ...
      context "when we cleanup dna files" do
        ...
        it 'cleanups dna files' do
          is_expected.to run_execute("Cleanup dna.json and extra.json from #{node['cluster']['update']['dna_dir']}").with(
            command: "#{cookbook_virtualenv_path}/bin/python #{node['cluster']['scripts_dir']}/manage_fleet_dna.py" \
