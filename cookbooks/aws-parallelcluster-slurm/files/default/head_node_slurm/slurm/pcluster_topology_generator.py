...
def generate_topology_config_file(output_file: str, input_file: str, block_sizes: str):
    ...
    BlockName=block1 Nodes=queue-1-st-compute-resource-0-[1-2] #### 2 nodes
    BlockName=block2 Nodes=queue-1-st-compute-resource-0-[3-6] #### 4 nodes
    BlockSizes=2,4
    """

    min_block_size_list = min(block_sizes.split(","))

    cluster_config = _load_cluster_config(input_file)
    queue_name, compute_resource_name = None, None
    try:
        topology_config = CONFIG_HEADER + "\n"
        for queue_id, queue_config in enumerate(cluster_config["Scheduling"]["SlurmQueues"]):
            ...
            for block_name_id, compute_resource_config in enumerate(queue_config["ComputeResources"]):
                ...
                ### Each Capacity Reservation ID is a Capacity Block and
                topology_config += "BlockName=Block" + str(queue_id+block_name_id)+ "  Nodes=" + str(queue_name) + "-" + str(node_type) + "-" + str(compute_resource_name) + "-[1-" + str(compute_max_count) + "] \n"
