...
COMPUTEFLEET_STATUS_FILE="computefleet-status.json"
...
DUMMY_COMPUTEFLEET_STATUS="${DUMMY_DIR}/${COMPUTEFLEET_STATUS_FILE}"
...
if [[ ! "${event_name}" =~ ^(HeadInit|HeadConfigure|HeadFinalize|ComputeInit|ComputeConfigure|ComputeFinalize|HeadClusterUpdate|HeadComputeFleetUpdate)$ ]]; then
...
if [[ "${event_name}" == "HeadComputeFleetUpdate" ]] && [[ -z ${computefleet_status} ]]; then
...
build_dummy_computefleet_status_json() {
...
if [[ "${event_name}" == "HeadComputeFleetUpdate" ]]; then
  build_dummy_computefleet_status_json
fi
build_dna_json
cleanup_previous_env
