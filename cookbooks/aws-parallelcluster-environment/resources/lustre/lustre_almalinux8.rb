...
provides :lustre, platform: 'almalinux' do |node|
...
action :setup do
  ...
  if current_version < Gem::Version.new("8.2")
    raise "FSx for Lustre is not supported in this AlmaLinux version #{version}, supported versions are >= 8.2"
    ...
    raise "FSx for Lustre is not supported in kernel version #{node['cluster']['kernel_release']} of AlmaLinux #{version}, please update the kernel version"
  else
