...
describe 'mysql_client:validate' do
  for_all_oses do |platform, version|
    %w(x86_64 aarch64).each do |architecture|
      context "on #{platform}#{version} #{architecture}" do
        cached(:expected_version) do
          if architecture == 'aarch64'
            '8.0.31'
          elsif architecture == 'x86_64'
            if platform == 'ubuntu'
        ...
        cached(:chef_run) do
          ...
          ConvergeMysqlClient.validate(runner)
        end
        cached(:node) { chef_run.node }

        before do
          allow_any_instance_of(Object).to receive(:validate_package_version) do
            # nothing
          end
        end

        it "validates packages" do
          repository_packages.each do |pkg|
            expect_any_instance_of(Object).to receive(:validate_package_version).with(pkg, expected_version)
          end
          chef_run
        end
      end
