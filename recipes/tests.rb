...
if node['cfncluster']['cfn_scheduler'] == 'slurm'
  ...
  when 'MasterServer'
    execute 'check-slurm-version' do
      ...
      environment('PATH' => '#{node['cfncluster']['slurm']['install_dir']}/bin:/bin:/usr/bin:$PATH')
      user node['cfncluster']['cfn_cluster_user']
    end
    ...
    execute 'check-slurm-accounting-mysql-plugins' do
      command "ls #{node['cfncluster']['slurm']['install_dir']}/lib/slurm/ | grep accounting_storage_mysql"
    end

    execute 'check-slurm-jobcomp-mysql-plugins' do
      command "ls #{node['cfncluster']['slurm']['install_dir']}/lib/slurm/ | grep jobcomp_mysql"
    end

    execute 'check-slurm-pmix-plugins' do
      command 'ls #{node['cfncluster']['slurm']['install_dir']}/lib/slurm/ | grep pmix'
    end
  ...
  when 'ComputeFleet'
    execute 'ls slurm root' do
      command "ls #{node['cfncluster']['slurm']['install_dir']}"
      user node['cfncluster']['cfn_cluster_user']
    end
  else
