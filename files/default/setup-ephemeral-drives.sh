...
function set_imds_token(){
  if [ -z "${IMDS_TOKEN}" ];then
    IMDS_TOKEN=$(curl -s -f -X PUT -H "X-aws-ec2-metadata-token-ttl-seconds: 900" http://169.254.169.254/latest/api/token)
...
# param1 = query
function get_meta() {
    local imds_out=$(curl -s -q -H "X-aws-ec2-metadata-token:${IMDS_TOKEN}" -f http://169.254.169.254/latest/${1})
    echo -n "${imds_out}"
}
